...
"""
        copy paste of https://github.com/python/cpython/blob/master/Lib/pyclbr.py
        to make code work with python3.6 and earlier versions

        also this pyclbr has modifications to show async methods in global scope and inside classes
            as separate AsyncFunction type

        async functions in classes nested in async_methods attr
...
class Class(_Object):
    ...
    def _add_async_method(self, name, lineno):
        self.async_methods[name] = lineno
...
def _nest_function(ob, func_name, lineno, async_f=False):
    ...
    if isinstance(ob, Class):
        if not async_f:\
            ...
            ob._add_async_method(func_name, lineno)
    return newfunc
        ...
        for tokentype, token, start, _end, _line in g:
            if tokentype == DEDENT:
                ...
                if stack:
                    ...
                    cur_func = AsyncFunction(fullmodule, func_name, fname, lineno)
